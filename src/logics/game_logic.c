...
void	hook(void* param)
{
	...
	move_player(game);
	cast_all_rays(game);
	draw_rect(game, (t_vec){0,0}, (t_vec){WIN_WIDTH, WIN_HEIGHT / 2}, rgba2int(game->map->cell->r,game->map->cell->g,game->map->cell->b, 255));
	draw_rect(game, (t_vec){0,WIN_HEIGHT / 2}, (t_vec){WIN_WIDTH, WIN_HEIGHT}, rgba2int(game->map->floor->r,game->map->floor->g,game->map->floor->b, 255));
	render_3d(game);
	// render_minimap(game);
